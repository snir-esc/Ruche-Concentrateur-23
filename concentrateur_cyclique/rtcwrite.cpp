...
void RTCWrite::set_startup_time(int s,int m ,int h, int d){   // L'écriture se fait au format BCD Donc chaque quartet 0-9
...
void RTCWrite::clear_startup_time(){
...
void RTCWrite::set_shutdown_time(int m, int h,int d){
...
void RTCWrite::clear_shutdown_time(){
...
void RTCWrite::set_next_startup_in(int m, int h){
...
void RTCWrite::set_next_shutdown_in(int m, int h){
...
vector<int> RTCWrite::ToUtc(int s,int m,int h, int d){
...
QDateTime RTCWrite::get_startup_time(){
    QDateTime DT;
    QDate Date;
    QTime Time;
    QByteArray ValeurqByte;
    QString qString;
    int s=0; int m=0; int h=0; int d=15;
    int Ds=0; int Dm=0; int Dh=0; int Dd=0;


    mProcess.start("i2cget", (QStringList){"-y","1","0x68","0x07"});
    mProcess.waitForFinished();
    ...
    mProcess.start("i2cget", (QStringList){"-y","1","0x68","0x08"});
    mProcess.waitForFinished();
    ValeurqByte = mProcess.readAll();
    qString.append(ValeurqByte);
    m=(int)qString.toInt(nullptr,16);
    ...
    mProcess.start("i2cget", (QStringList){"-y","1","0x68","0x09"});
    mProcess.waitForFinished();
    ValeurqByte = mProcess.readAll();
    qString.append(ValeurqByte);
    h=(int)qString.toInt(nullptr,16);
    ...
    mProcess.start("i2cget", (QStringList){"-y","1","0x68","0x0A"});
    mProcess.waitForFinished();
    ValeurqByte = mProcess.readAll();
    qString.append(ValeurqByte);
    d=(int)qString.toInt(nullptr,16);
